...
export default function ChartPiePromotores() {
...
for(i=0;i<NpsPie.length;i++){
    ...
    if (NpsPie[i].promFeature.includes('Gastronomía')){
      gastro++;
    } 
    
    if (NpsPie[i].promFeature.includes('Máquinas de Slots')){
      maq++;}

    if (NpsPie[i].promFeature.includes('Otros')){
      otros++;}

    if (NpsPie[i].promFeature.includes('Limpieza')){
      lim++;}

    if (NpsPie[i].promFeature.includes('Seguridad')){
      seg++;}
}
  ...
  return (
    ...
    <Card className='npsCard' bg={'light'} style={{ margin:'20px auto', textAlign:'center', color:'black', fontSize:'30px'  }}>
      <Card.Header>Valoraciones Promotores</Card.Header>
      <Card.Body>
